...
export async function runProductResearch(productName: string, productCategory: string): Promise<Record<string, string>> {
  ...
  const queries = [
    ...
    { key: 'influencers', query: `${productName} ${productCategory} influencer marketing instagram youtube` },
    { key: 'content_marketing', query: `${productName} ${productCategory} content marketing ideas social media strategy` }
  ];
