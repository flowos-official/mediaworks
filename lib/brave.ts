...
export async function runProductResearch(
  ...
  if (locale === 'ja') {
    ...
    const queries = [
      ...
      // 日本輸出適合性 (追加)
      { key: 'japan_market', query: `${productName} 日本 ホームショッピング 市場規模` },
      { key: 'japan_price', query: `${productName} 楽天 価格帯 相場` },
      { key: 'japan_reviews', query: `${productName} Amazon Japan レビュー 評価` },
    ];
    ...
    const queries = [
      ...
      // Japan market queries (always included)
      { key: 'japan_market', query: `${productName} 日本 ホームショッピング 市場規模` },
      { key: 'japan_price', query: `${productName} 楽天 価格帯 相場` },
      { key: 'japan_reviews', query: `${productName} Amazon Japan レビュー 評価` },
    ];
