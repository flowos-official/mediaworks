...
=== プール出典について — 重要 ===
プールには楽天/Amazon 由来の商品と、TV放送局公式オンラインショップ (japanet/shop.ntv.co.jp/dinos/ropping/kachimo/tbs等) 由来の商品が混在している。
TV放送局公式サイト由来の商品はレビュー数/評価を公開していないことが多い。これは「データ欠落」であって「品質低」ではない。放送局のバイヤーが既にオンエア候補として選定した時点で強い社会的証明であり、レビュー欠落を理由に減点しないこと。

=== japan_fit_score 採点ルール (0-100) — ソース中立 ===
以下の加点で算出すること。各カテゴリで該当する一段階のみ加点 (重複加点禁止):
- 楽天レビュー数: ≥100件→+10 / 50-99件→+7 / 5-49件→+3 / それ以下→0 / レビュー非公開(TV放送局公式サイト等)→+5 (ニュートラル)
- 楽天レビュー平均: ≥4.0→+8 / 3.5-3.9→+5 / それ未満→0 / 評価非公開→+4 (ニュートラル)
- TVトップカテゴリ一致: 一致→+30 / 隣接→+15 / 不一致→0  (最重要シグナル)
- 日本市場トレンド情報に関連語句あり: あり→+15 / なし→0
- ユーザー目標/ターゲット市場合致: 合致→+12 / 不合致→0
- TV通販/ライブ配信実演適合性 (映像デモ可能・衝動買い価格帯・ギフト需要): 高→+25 / 中→+12 / 低→0
- 合計は必ず 0-100 の範囲に収めること (上限超えは100に丸める)

=== スコアリング哲学 — 厳守 ===
- forward-looking シグナル (TVカテゴリ一致 + 実演適合性 + ユーザー目標合致 = 67点) を主軸に、過去データ (レビュー + トレンド = 33点) は補助。
- レビュー欠落で機械的に減点せず、ニュートラル値 (約9点) を与える。
- 商品の TV/EC 適性そのもので競わせる。結果のソース分布 (楽天:TV放送局) は質次第で自然に決まる。
