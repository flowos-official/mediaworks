...
- **Insight refresh** (`lib/intelligence/{insights,refresh-insights}.ts`, cron `/api/cron/refresh-intelligence-insights`, JST 05:00): incrementally turns active evidence into `insight_snapshots`, each linked to the exact evidence rows it consumed via `insight_snapshot_evidence`.
