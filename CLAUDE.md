...
- Archival daily procedure (TEMPORARY — Vercel production deploy is currently blocked by an unrelated `workflow` plugin × Vercel CLI 53.x bug on dynamic-route symlinks, see git history around 2026-05-19): until production cron resumes, run `npm run daily:archive` locally once a day after JST 01:30. This chains `backfill:broadcast-products` (enriches yesterday's whitelist-matching slots — sets `brand_name`, fills `broadcast_products`, marks `video_status='queued'`) + `drain:archive-queue` (loops `archiveOne` over the queue at concurrency 4 until empty). Typical daily run is ~3-5 min for 20-30 new slots. After the Vercel deploy is unblocked and `app/api/cron/archive-videos/route.ts` is live, this manual step can be retired.
