...
`deep_dive` click keeps its existing `+0.5`. The graded funnel matters because `aired` is sparse and lagged; `sourcing`/`scheduled` are denser and arrive sooner, so the loop gets useful signal long before airings accumulate.
