...
- **Depends on**: 기존 Discovery 피드백 루프 (Phase 4 완료), Vercel Workflow DevKit
- **Out of scope**: 자동 A/B winner 선정, LLM 메타 플래너, full multi-agent debate framework (별도 spec), **Vercel AI Gateway 도입 (사용자 결정: 직접 Gemini SDK 유지)**
...
3. 신규 skill 추가가 데이터 작업으로 가능하게 한다 (단, prompt 본문은 git 소스 truth 유지 — §4.1 참조)
...
- 모든 Gemini 호출이 `callGeminiWithTelemetry()` 래퍼를 거쳐 토큰·비용·지연이 자동 캡처된다.

### 1.2 범위 밖 (Out of Scope)

- **Vercel AI Gateway** — 직접 Gemini SDK 유지 결정. 토큰·비용 텔레메트리는 §6.1 의 manual wrapper 로 직접 캡처.
...
- DSPy / MIPRO 식 자동 prompt 최적화 (sample size 부족; §12 외부 참고)
...
```
              ...
              read-only   │             │  per-run write
                ...
                └─────────────┘  └────────┬─────────┘
                                          ▼
                              ┌────────────────────────┐
                              │ callGeminiWithTelemetry│ ← 토큰/비용/지연 capture
                              └─────────┬──────────────┘
                                        ▼
                              ┌────────────────────────┐
                              │ @google/genai          │
...
- 2025-2026 산업 합의 (Anthropic, Cursor, Linear): DB-only 편집은 "3 단어 prompt 수정 → 수 시간 내 JSON parse cascade" 같은 인시던트를 양산
...
lib/registry/
  ...
  telemetry.ts                   # callGeminiWithTelemetry() 래퍼 (§6.1)
  pricing.ts                     # 모델별 단가 테이블 (§6.2)
  skills/
    goal_analysis/
      v1/
...
CREATE TABLE skill_versions (
  ...
  model text NOT NULL,                       -- 'gemini-3-flash-preview' (Google SDK 모델 ID)
  provider text NOT NULL DEFAULT 'google',   -- 'google' | 'anthropic' (Phase D)
  generation_config jsonb NOT NULL,          -- { temperature, maxTokens, thinkingConfig }
...
CREATE TABLE skill_runs (
  ...
  input_jsonb jsonb,
  output_jsonb jsonb,
  tokens_in int,                             -- response.usageMetadata.promptTokenCount
  tokens_out int,                            -- response.usageMetadata.candidatesTokenCount
  cost_usd numeric(10,6),                    -- lib/registry/pricing.ts 단가표로 계산
  duration_ms int,                           -- Date.now() wrap
  validator_violations jsonb DEFAULT '[]',
  status text NOT NULL,
  error_message text,                        -- 실패 시 보존
  started_at timestamptz NOT NULL DEFAULT now(),
  finished_at timestamptz,
...
AI Gateway 미사용 결정에 따라 토큰·비용·지연을 직접 캡처. 한 곳에 모아두는 wrapper 와 단가 테이블 두 모듈로 분리.

### 6.1 `lib/registry/telemetry.ts` — Gemini 호출 wrapper

```typescript
import { GoogleGenAI, type GenerateContentResponse } from "@google/genai";
import { computeCost } from "./pricing";
import { insertSkillRun } from "./runs";
...
export interface CallContext {
  ...
  stepName: string;
  inputHash: string;
  inputJsonb?: Record<string, unknown>;
}
...
export async function callGeminiWithTelemetry(args: {
  model: string;                              // 'gemini-3-flash-preview' 등 Google SDK 모델 ID
  prompt: string;
  ...
  const costUsd = computeCost(args.model, tokensIn, tokensOut);
  const text = response?.text ?? "";

  // Fire-and-forget; failure to record telemetry must not crash skill execution
  void insertSkillRun({
    agent_run_id: args.ctx.agentRunId,
    skill_id: args.ctx.skillId,
    skill_version_id: args.ctx.skillVersionId,
    step_name: args.ctx.stepName,
    input_hash: args.ctx.inputHash,
    input_jsonb: args.ctx.inputJsonb,
    output_jsonb: text ? { text } : null,
    tokens_in: tokensIn,
    ...
    console.warn("[telemetry] insertSkillRun failed:", err instanceof Error ? err.message : err)
  );

  if (status === "failed") throw new Error(errorMessage);
  return { text, tokensIn, tokensOut, costUsd, durationMs };
}
```

### 6.2 `lib/registry/pricing.ts` — 모델별 단가 테이블

```typescript
// USD per 1M tokens, 2026-05 기준. 가격 변동 시 한 곳만 갱신.
// 출처: https://ai.google.dev/pricing, https://www.anthropic.com/pricing
const PRICING: Record<string, { input: number; output: number }> = {
  // Google Gemini
  "gemini-3-flash-preview": { input: 0.30, output: 2.50 },
  "gemini-3.1-pro-preview": { input: 1.25, output: 10.0 },
  // Anthropic (Phase D critic)
...
export function computeCost(model: string, tokensIn: number, tokensOut: number): number {
  ...
  if (!rate) {
    console.warn(`[pricing] unknown model: ${model}; cost = 0`);
    return 0;
  }
...
### 6.3 Phase D — Cross-family critic 시 추가

`@anthropic-ai/sdk` 별도 install 필요. 같은 telemetry wrapper 패턴을 `callClaudeWithTelemetry()` 로 미러:

```typescript
// lib/registry/telemetry-anthropic.ts (Phase D 시점에 추가)
import Anthropic from "@anthropic-ai/sdk";
const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY! });
// 구조는 callGeminiWithTelemetry 와 동일
```

`skill_versions.provider` 컬럼이 `'google' | 'anthropic'` 으로 라우팅 분기 결정.
...
async function publishRegistry() {
  ...
  for (const skill of skills) {
    ...
    const versionRow = await insertSkillVersionIfNew({
      ...
      provider: skill.meta.provider ?? 'google',
...
## 8. Pipeline Runner — `runPipeline(agentSlug, input)`
...
export async function runPipeline(
  ...
  for (const node of order) {
    ...
    // 내부적으로 callGeminiWithTelemetry() 가 토큰·비용·skill_runs 기록까지 처리
    const result = await runSkill(skillVersion, { input, outputs, agentRunId: run.id });
    outputs[node.skill_slug] = result.output;
  }

  // 4. agent_runs 마무리 — skill_runs 집계
  await rollupAgentRun(run.id);

  return { agentRunId: run.id, outputs };
}
```

`runSkill` 내부에서 `skillVersion.provider === 'google'` 이면 `callGeminiWithTelemetry`, `'anthropic'` 이면 `callClaudeWithTelemetry` 로 분기.
...
|---|---|---|---|
| **Registry list** | `/admin/registry` | `skills` ⋈ active `skill_versions` ⋈ `skill_version_cost_30d` | 정렬·필터 가능 테이블: slug, 활성 버전, 모델, p95 latency, 30일 비용 (USD), violation rate, failure rate |
...
| **Run inspector** | `/admin/runs/[agentRunId]` | `agent_runs` row + `skill_runs` 트리 | react-flow DAG 시각화, 노드 클릭 시 drawer 에 input/output JSON / violations / tokens / cost |
...
이 spec 의 registry 가 도입되면 이전 분석에서 도출된 Phase 들이 자연스럽게 맞물림:

| Phase | Registry 와의 관계 |
|---|---|
| **Phase A** Telemetry + 결정론적 validator | `callGeminiWithTelemetry` (§6.1) 가 토큰·비용·지연 자동 캡처 → `skill_runs` 에 기록. `skill_versions.output_schema` 가 Zod → JSON Schema 변환본. 런타임 후처리 검증 결과를 `skill_runs.validator_violations` 에 기록 |
...
| **Phase D** Evaluator-Optimizer (critic) | `validator_critic` 라는 skill 슬러그로 registry 등록. `provider='anthropic'` 으로 표시 → cross-family judge (Claude Sonnet 4.6) 사용. `agent_pipelines.dag` 의 한 노드로 삽입. `@anthropic-ai/sdk` 별도 install + `ANTHROPIC_API_KEY` env 추가 |

---

## 11. 거부하는 anti-patterns (명시적)
...
| Gemini-judges-Gemini critic | self-preference bias (arXiv 2508.06709). `validator_critic` 의 `provider` 필드를 `'anthropic'` 으로 강제. `meta.ts` 의 `provider` 필드 검증 게이트로 publish 차단 |
...
| Skill 호출을 telemetry wrapper 없이 직접 | 토큰·비용 추적 누락. `callGeminiWithTelemetry` 우회는 lint rule + grep 게이트로 차단 |

---

## 12. 마이그레이션 순서 (Step 1-13, 각 단계 독립 출시 가능)
...
| 8 | `lib/registry/telemetry.ts` + `pricing.ts` 작성 (§6) | 7 | 1일 |
| 9 | 기존 for-loop 에 dual-write 추가 (실행은 옛 코드, telemetry wrapper 와 skill_runs 기록만 신규) | 7, 8 | 1-2일 |
| 10 | Admin UI 페이지 3+4 (run inspector, pipeline view) | 9 | 3일 |
| 11 | `lib/registry/runner.ts` 의 `runPipeline()` 구현 | 6, 8 | 2일 |
...
- **Phase D**: `validator_critic` skill 등록 (`provider='anthropic'`) + Evaluator-Optimizer 패턴 + `workflow_decisions` 캐시 + `@anthropic-ai/sdk` install
...
| PR | 범위 | 추정 |
|---|---|---|
| **PR α** (this) | Registry 설계 spec 문서 | (작성 중) |
| **PR β** | Step 1-2: registry DDL migration + immutability trigger + RLS + `lib/registry/` 스캐폴드 + publish CI | 1-2일 |
| **PR γ** | Step 3: `goal_analysis` 만 v1 으로 이관해서 publish 파이프라인 end-to-end 검증 | 반일 |
| **PR δ** | Step 4: 나머지 16개 skill 일괄 v1 이관 | 2-3일 |
| **PR ε** | Step 5: Admin UI 페이지 1+2 (registry list + skill detail) → 사용자 요구 "보이는 형태" 직접 충족 | 3일 |

PR β + γ + ε 까지면 **약 1주 안에 처음 사용자에게 "registry 가 살아 있다"가 보이는 상태** 가 됨.
...
| [arXiv 2508.06709 — Self-bias in LLM judge](https://arxiv.org/abs/2508.06709) | Same-family judge 의 self-preference bias 정량 증거. cross-family judge (`provider='anthropic'`) 채택 근거 |
...
| [Google Gemini API pricing](https://ai.google.dev/pricing) | `lib/registry/pricing.ts` 단가표 원천 |
| [Anthropic pricing](https://www.anthropic.com/pricing) | Phase D 시점 Claude 단가 |
| [vercel/workflow-examples](https://github.com/vercel/workflow-examples) | WDK 캐노니컬 패턴 |

---

## 15. Open questions (사용자 확인 필요)
...
2. **Phase D Anthropic API 예산**: cross-family critic 으로 Claude Sonnet 4.6 사용 시 input $3/1M / output $15/1M. 7 skill × 30 runs/월 = ~210 critic calls/월. 평균 2K input + 500 output 가정 시 월 ~$3-5. 부담 적지만 사전 승인 필요
3. **`analyze_expansion_strategy` (legacy) 처리**: registry 에 넣을지, deprecate 후 사용처 정리할지?
4. **Discovery `enrich_product` 는 실제로 다단 agent**: tool calling 까지 포함됨. 일단 단순 prompt 로 평탄화해 registry 에 v1 publish 하고, tool-calling 지원은 별도 spec 으로 미룰지?
5. **모델 단가 변경 추적**: `lib/registry/pricing.ts` 가 git 에 있어 PR 로 추적되지만, Google/Anthropic 이 단가를 바꾸면 인지 시점이 늦어질 수 있음. 분기별로 docs 자동 fetch 해서 변경 감지하는 잡을 둘지?
