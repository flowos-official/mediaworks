...
function roiColor(pct: number): string {
	if (pct >= 100) return 'text-green-700 dark:text-green-300 bg-green-600/10';
	if (pct >= 50) return 'text-blue-700 dark:text-blue-300 bg-blue-600/10';
	if (pct >= 0) return 'text-yellow-700 dark:text-yellow-300 bg-yellow-500/10';
	...
	return (
		<div className="space-y-4">
			<div className="flex items-center gap-2">
				...
				<h3 className="text-lg font-bold text-foreground">収益予測</h3>
			</div>
					...
					<CardContent>
						<div className="space-y-2">
							{(data.roi_timeline ?? [])
								...
								.map((roi) => (
									<div key={roi.channel} className="bg-muted rounded-lg px-3 py-2.5">
										<div className="flex items-center justify-between mb-1">
											<span className="font-semibold text-sm text-foreground">{roi.channel}</span>
										...
										<div className="grid grid-cols-3 gap-2 text-xs">
											<div>
												<span className="text-muted-foreground block text-[9px]">総投資額</span>
												<span className="font-mono font-medium">{formatYen(roi.total_investment)}</span>
											</div>
											<div>
												<span className="text-muted-foreground block text-[9px]">損益分岐</span>
												<span className="font-medium">{roi.breakeven_month}</span>
											</div>
											<div>
												<span className="text-muted-foreground block text-[9px]">1年目純利益</span>
			...
			<Card className="border-blue-600/30 bg-blue-600/5">
				<CardHeader className="pb-2">
					<CardTitle className="text-sm font-semibold text-blue-700 dark:text-blue-300">シナリオ分析</CardTitle>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-3 gap-3 mb-3">
						{[
							{ label: '保守的', data: data.scenarios.conservative, color: 'border-border bg-muted' },
							{ label: '中立的', data: data.scenarios.moderate, color: 'border-blue-600/40 bg-blue-600/10' },
							{ label: '積極的', data: data.scenarios.aggressive, color: 'border-green-600/40 bg-green-600/10' },
							...
							<div key={s.label} className={`rounded-lg border p-3 text-center ${s.color}`}>
								...
								<div className="mt-1">
									<div className="text-lg font-bold text-foreground">{formatYen(s.data.year1_revenue)}</div>
									<div className="text-xs text-muted-foreground">売上</div>
								</div>
								<div className="mt-1">
									<div className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">{formatYen(s.data.year1_profit)}</div>
									<div className="text-[10px] text-muted-foreground">利益</div>
								</div>
							</div>
						))}
					</div>
					{(data.scenarios.assumptions ?? []).length > 0 && (
						<div>
							<span className="text-[10px] text-muted-foreground font-semibold">前提条件:</span>
