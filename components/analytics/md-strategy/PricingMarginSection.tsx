...
function marginColor(pct: number): string {
	if (pct >= 30) return 'text-green-700 dark:text-green-300 bg-green-600/10';
	if (pct >= 15) return 'text-blue-700 dark:text-blue-300 bg-blue-600/10';
	if (pct >= 5) return 'text-yellow-700 dark:text-yellow-300 bg-yellow-500/10';
	...
	return (
		<div className="space-y-4">
			<div className="flex items-center gap-2">
				...
				<h3 className="text-lg font-bold text-foreground">価格・マージン戦略</h3>
			</div>

			{/* Product pricing table per product */}
			{(data.product_pricing ?? []).map((pp) => (
				<Card key={pp.product_code} className="border-border">
					<CardHeader className="pb-2">
						<CardTitle className="text-sm font-bold">{pp.product_name}</CardTitle>
						<div className="flex gap-3 text-xs text-muted-foreground">
							{pp.cost_basis.cost_price > 0 && <span>原価: <span className="font-mono font-medium text-foreground">¥{pp.cost_basis.cost_price.toLocaleString()}</span></span>}
							{pp.cost_basis.wholesale_rate > 0 && <span>卸売率: <span className="font-mono font-medium text-foreground">{pp.cost_basis.wholesale_rate}%</span></span>}
							{pp.cost_basis.current_tv_price > 0 && <span>TV単価: <span className="font-mono font-medium text-foreground">¥{pp.cost_basis.current_tv_price.toLocaleString()}</span></span>}
						</div>
					</CardHeader>
					<CardContent className="pt-0">
							...
							<table className="w-full text-xs">
								...
								<tbody>
									{(pp.channel_pricing ?? []).map((cp) => (
										<tr key={cp.channel} className="border-b border-border hover:bg-muted/50">
											<td className="px-2 py-2 font-medium text-foreground">{cp.channel}</td>
											<td className="px-2 py-2 text-right font-mono font-medium">¥{cp.recommended_price.toLocaleString()}</td>
											<td className="px-2 py-2 text-right text-muted-foreground">{cp.competitor_benchmark}</td>
											<td className="px-2 py-2 text-right text-muted-foreground">{cp.channel_fees}</td>
						...
						{pp.channel_pricing[0]?.reasoning && (
							<p className="text-[10px] text-muted-foreground mt-2 italic">{pp.channel_pricing[0].reasoning}</p>
						)}
					</CardContent>
				</Card>
			))}
				...
				<Card className="border-emerald-600/30 bg-emerald-600/5">
					...
					<CardContent className="space-y-3">
						{(data.bep_analysis ?? []).map((bep) => (
							<div key={bep.channel} className="bg-card rounded-lg border border-emerald-600/20 p-3">
								<div className="flex items-center justify-between mb-2">
									<span className="font-semibold text-sm text-foreground">{bep.channel}</span>
									<Badge className="bg-emerald-600 text-white text-[10px]">BEP: {bep.bep_units}個/月</Badge>
								</div>
								<div className="grid grid-cols-3 gap-2 text-xs mb-2">
									<div className="bg-muted rounded px-2 py-1">
										<span className="text-muted-foreground block text-[9px]">変動費/個</span>
										<span className="font-mono font-medium">¥{bep.variable_cost_per_unit.toLocaleString()}</span>
									</div>
									<div className="bg-muted rounded px-2 py-1">
										<span className="text-muted-foreground block text-[9px]">BEP売上</span>
										<span className="font-mono font-medium">¥{bep.bep_revenue.toLocaleString()}</span>
									</div>
									<div className="bg-muted rounded px-2 py-1">
										<span className="text-muted-foreground block text-[9px]">達成見込</span>
