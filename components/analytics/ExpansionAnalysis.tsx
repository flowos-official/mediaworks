...
function EvidenceSection({ topProducts, categorySummary }: {
  ...
  return (
    <Card className="border-emerald-600/30 bg-emerald-600/5">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-semibold flex items-center gap-1.5 text-emerald-700 dark:text-emerald-300">
          <Database size={14} /> データ根拠 — TV通販実績
        </CardTitle>
        <p className="text-[10px] text-muted-foreground">以下の販売実績データを基に上記の分析が導出されました</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Top products table */}
        <div>
          <span className="text-[10px] font-semibold text-muted-foreground uppercase">上位商品実績</span>
          <div className="overflow-x-auto mt-1">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-emerald-600/20 text-muted-foreground">
              ...
              <tbody>
                {topProducts.slice(0, 10).map((p, i) => (
                  <tr key={p.code} className="border-b border-border">
                    ...
                    <td className="px-2 py-1 text-foreground font-medium max-w-[160px] truncate">{p.name}</td>
                    <td className="px-2 py-1">
                      {p.category && <span className="text-[9px] px-1 py-0.5 rounded bg-muted">{p.category}</span>}
                    </td>
                    ...
                    <td className="px-2 py-1 text-right font-mono text-muted-foreground">{p.weekCount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Category chart */}
        <div>
          <span className="text-[10px] font-semibold text-muted-foreground uppercase">カテゴリ別売上構成 (万円)</span>
    ...
    <div className="space-y-6">
        ...
        <Card className="border-border mb-4">
          <CardContent className="p-4">
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-border bg-background text-foreground rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 resize-none"
            />
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 space-y-3">
          ...
          <p className="text-sm text-muted-foreground">TV通販の販売実績データをAIが分析中...</p>
          <p className="text-[10px] text-muted-foreground">161商品 × 45週間のデータを基に最適なチャネル戦略を導出しています</p>
        </div>
      )}
        ...
        <>
          ...
          <Card className="border-blue-600/30 bg-blue-600/10">
            <CardHeader className="pb-1">
              <CardTitle className="text-sm font-semibold text-blue-700 dark:text-blue-300">総合分析</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  return (
                    <Card key={ch.channel} className="border-border">
                      ...
                      <CardContent className="pt-0 space-y-4">
                        {/* Why this channel */}
                        <div>
                          <span className="text-[10px] font-semibold text-muted-foreground uppercase block mb-1">なぜこのチャネルか</span>
                          <p className="text-sm text-foreground leading-relaxed">{ch.reasoning}</p>
                        </div>

                        {/* Recommended products with evidence */}
                        {ch.recommended_products && ch.recommended_products.length > 0 && (
                          <div>
                            <span className="text-[10px] font-semibold text-muted-foreground uppercase block mb-1.5">推奨商品 — TV通販実績に基づく根拠</span>
                            <div className="space-y-2">
                              {ch.recommended_products.map((p: RecommendedProduct | string, i: number) => {
                                if (typeof p === 'string') {
                                  return (
                                    <div key={i} className="bg-blue-600/10 rounded-lg px-3 py-2 text-xs">
                                      <span className="font-medium text-foreground">{p}</span>
                                    </div>
                                  );
                                }
                                return (
                                  <div key={i} className="bg-blue-600/10 border border-blue-600/20 rounded-lg px-3 py-2.5">
                                    <div className="flex items-center justify-between">
                                      <span className="font-semibold text-sm text-foreground">{p.name}</span>
                                    </div>
                                    <div className="flex items-center gap-3 mt-1.5 text-xs">
                                      <span className="bg-card px-2 py-0.5 rounded border border-blue-600/30 font-mono text-blue-700 dark:text-blue-300">TV売上 {p.tv_revenue}</span>
                                      <span className="bg-card px-2 py-0.5 rounded border border-green-600/30 font-mono text-green-700 dark:text-green-300">粗利率 {p.margin}</span>
                                      <span className="bg-card px-2 py-0.5 rounded border border-orange-600/30 font-mono text-orange-700 dark:text-orange-300">週平均 {p.weekly_avg}個</span>
                                    </div>
                                    {p.fit_reason && (
                                      <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">→ {p.fit_reason}</p>
                                    )}
                                  </div>
                                );
                              })}
                        ...
                        {strategy && (
                          <div className="border-t border-border pt-3">
                            <span className="text-[10px] font-semibold text-muted-foreground uppercase block mb-1.5">具体的な参入ステップ</span>
                            <div className="flex gap-3 mb-2 text-xs">
                              <span className="bg-muted px-2 py-0.5 rounded font-medium">期間: {strategy.timeline}</span>
                              <span className="bg-muted px-2 py-0.5 rounded font-medium">初期投資: {strategy.initial_investment}</span>
                            </div>
                        ...
                        {risk && (
                          <div className="bg-orange-600/10 border border-orange-600/20 rounded-lg p-3">
                            <span className="text-[10px] font-semibold text-orange-600 dark:text-orange-300 uppercase flex items-center gap-1 mb-1">
                            ...
                            <p className="text-xs text-foreground bg-card rounded px-2 py-1.5 border border-orange-600/20">
                              <span className="font-medium text-orange-700 dark:text-orange-300">対策:</span> {risk.mitigation}
                            </p>
                          </div>
                        )}
                      </CardContent>
              ...
              <CardContent>
                <div className="space-y-2">
                  {analysis.product_channel_fit.map((pcf) => (
                    <div key={pcf.product} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
                      <span className="text-xs font-medium text-foreground min-w-[140px] shrink-0">
                        {pcf.product}
                      </span>
                      ...
                      <span className="text-[10px] text-muted-foreground ml-auto max-w-[200px]">{pcf.reasoning}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
