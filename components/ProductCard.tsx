...
import { FileText, Clock, CheckCircle, Loader2, AlertCircle, ArrowRight } from 'lucide-react';
...
const statusConfig: Record<string, { icon: typeof Clock; color: string; label: string }> = {
  ...
  extracted: { icon: Loader2, color: 'bg-blue-100 text-blue-700', label: 'analyzing' },
...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const isProcessing = product.status === 'analyzing' || product.status === 'extracted';
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${config.color}`}>
              <Icon size={12} className={product.status === 'analyzing' ? 'animate-spin' : ''} />
