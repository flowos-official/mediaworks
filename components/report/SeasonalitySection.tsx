    ...
    <Card>
      ...
      <CardContent>
                ...
                <div className="h-20 w-full flex items-end">
                  <div
                    className={`w-full rounded-t-sm transition-all ${
                      isHigh ? 'bg-orange-500' : isMed ? 'bg-orange-300' : 'bg-orange-600/10'
                    }`}
                    style={{ height: `${Math.max(height, 8)}%` }}
                  />
                </div>
                <span className="text-xs text-muted-foreground">{t(month as 'jan')}</span>
        ...
        <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-orange-600/10 inline-block" /> Low (&lt;40)</span>
        </div>
      </CardContent>
    </Card>
  );
